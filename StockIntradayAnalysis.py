...
def get_sp500_components():
    ...
    extended_symbols = ['RIVN', 'AVGO', 'SPY', 'QQQ', 'TSLA', 'MA', 'BITO','BTC-USD', 'NIO', 'LCID', 'GME', 'SPCE', 'NKLA', 'MSTR']
    extended_companies = ['Rivian Automotive', 'Broadcom Inc', 'SPDR S&P 500 ETF', 'Invesco QQQ Trust', 'Tesla', 'Mastercard', 'BITO','BTCUSD', 'NIO', 'LUCID', 'GME', 'virgin galactic', 'Nikola', 'Microstrategy']
...
#st.dataframe(stock_df[['Open','High','Low','Close','Volume']].tail(5),hide_index=False)
    #temp_df = stock_df.copy()
     ...
     crypto_list = ['BTC-USD', 'MSTR', 'BITX', 'BITO', 'ARKB', 'MARA', 'YBTC', 'IBIT', 'YMAX']
     ...
     crypto_cp['BTC_IBIT'] = round(((crypto_cp['BTC-USD'] / 1000) / crypto_cp['IBIT']), 2)
     crypto_cp['BTC_YMAX'] = round(((crypto_cp['BTC-USD'] / 1000) / crypto_cp['YMAX']), 2)
        # Suppress the time part and display just the date portion
